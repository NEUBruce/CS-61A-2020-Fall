...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        ...
        if gain > running_high:
            ...
            return announce_highest(who,gain + last_score, gain)
        else:
            return announce_highest(who,gain + last_score, running_high)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_average = 0
    while count <= 10:
        tmp = make_averaged(roll_dice, trials_count)(count,dice)
        if tmp > max_average:
            max_average = tmp
            max_roll = count
        count += 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tmp = score
    if extra_turn(tmp + free_bacon(opponent_score), opponent_score):
        return 0
